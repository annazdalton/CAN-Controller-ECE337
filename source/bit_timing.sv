module can_timing_unit (
    ...
    always_comb begin
        ...
        if (rx_edge && rx_active && resync_enable && !resync_done_this_bit) begin
            if (tq_count < sample_tq) begin
                // early edge
                phase_error = sample_tq - tq_count;
            end else begin
                // late edge
                phase_error = tq_count - sample_tq;
            end
        ...
        if (!n_rst) begin
            ...
            if (enable && bus_idle && rx_falling_edge) begin
                ...
                if (rx_edge && rx_active && resync_enable && !resync_done_this_bit) begin
                    ...
                    end else if (tq_count > sample_tq) begin
